-- ============================================================
-- ⚠️ NUCLEAR OPTION: 推倒重建 public schema
-- ============================================================
-- 适用场景:
--   - 老的 public.users 表是错的 schema (有 password_hash, 没 openid/role)
--   - 没有任何真实业务数据 (orders/bloggers/commissions 全是 0 行)
--   - 想从零开始
--
-- 保留:
--   ✅ auth.users (你的 admin 账号 id=6f41de35-... 不会丢)
--   ✅ auth.identities / auth.sessions
--
-- 删除:
--   ❌ public.users / orders / wills / lawyers / ... 所有 public.* 表
--   ❌ 所有自定义函数 / 触发器 / 策略
--
-- 用法:
--   1. 先 SELECT * FROM public.users 确认没重要数据
--   2. 复制本文件内容到 Supabase SQL Editor → Run
--   3. 验证: SELECT * FROM information_schema.tables WHERE table_schema='public'
--   4. UPDATE public.users SET role='admin' WHERE id='6f41de35-...'
-- ============================================================
...
DROP TABLE IF EXISTS public.bloggers CASCADE;
DROP TABLE IF EXISTS public.lawyer_bookings CASCADE;
DROP TABLE IF EXISTS public.wechat_cs_sessions CASCADE;
...
-- ============================================================
-- 重建: 13 个 migrations 全部内容 (IF NOT EXISTS 保护)
-- ============================================================
