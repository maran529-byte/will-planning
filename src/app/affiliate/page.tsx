      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          <div className="bg-white rounded-xl p-5 shadow-sm border border-amber-100">
            ...
            <div className="text-sm text-slate-600 mt-1">基础佣金比例</div>
            <div className="text-xs text-slate-400 mt-2">专家套餐 ¥999 → 返 ¥99.9</div>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-amber-100">
            <div className="text-3xl font-bold text-amber-600">15%</div>
            <div className="text-sm text-slate-600 mt-1">高单价套餐佣金</div>
            <div className="text-xs text-slate-400 mt-2">专业套餐 ¥4699 → 返 ¥704.85</div>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-amber-100">
            <div className="text-3xl font-bold text-amber-600">T+7</div>
            <div className="text-sm text-slate-600 mt-1">结算周期</div>
            <div className="text-xs text-slate-400 mt-2">支付后 7 天变可提现</div>
          </div>
        </div>

        {/* 流程介绍 */}
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm mb-10">
          <h2 className="text-xl font-bold text-slate-800 mb-5">📋 结算规则</h2>
          <ol className="space-y-3 text-sm sm:text-base text-slate-700">
            <li className="flex gap-3">
              ...
              <span>
                ...
                <code className="px-1.5 py-0.5 mx-1 rounded bg-slate-100 text-xs">aiwill-planner.cn/?ref=您的推广码</code>
                进入, 30 天内下单均算您的推广业绩.
              </span>
            </li>
            <li className="flex gap-3">
        ...
        <div className="mt-10 text-center text-sm text-slate-500">
          有疑问? 微信联系客服 <span className="font-mono text-slate-700">wxid_xxx</span>
          {' '}(或邮件 affiliate@aiwill-planner.cn)
        </div>
      </div>
    </div>
  );
...
function ExistingBloggerStatus({ blogger }: { blogger: NonNullable<Awaited<ReturnType<typeof getBloggerByUserId>>> }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm">
      <h2 className="text-xl font-bold text-slate-800 mb-4">📝 我的申请</h2>

      <div className={`rounded-lg p-4 ${s.cls} mb-4`}>
      ...
      <dl className="space-y-3 text-sm">
        ...
        <div className="flex gap-2">
          ...
          <dd className="font-mono text-slate-800">{blogger.contact_phone || '-'}</dd>
        </div>
        <div className="flex gap-2">
        ...
        {blogger.ref_code && (
          <div className="flex gap-2 items-center bg-amber-50 -mx-2 px-3 py-2 rounded">
            ...
            <dd>
              <code className="font-mono font-bold text-amber-900 text-base">{blogger.ref_code}</code>
            </dd>
          </div>
        )}
        {blogger.commission_rate && (
          <div className="flex gap-2">
            ...
            <dd className="text-slate-800">{(blogger.commission_rate / 100).toFixed(1)}%</dd>
          </div>
        )}
        {blogger.review_note && (
          <div className="flex gap-2">
        ...
        <div className="flex gap-2">
          ...
          <dd className="text-slate-600 text-xs">{new Date(blogger.applied_at).toLocaleString('zh-CN')}</dd>
        </div>
      </dl>
